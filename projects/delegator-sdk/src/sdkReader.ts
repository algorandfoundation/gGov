...
import {
  ...
  DelegatorCommittee,
  DelegatorProposal,
} from './generated/DelegatorClient'
...
export class XGovDelegatorReaderSDK {
  ...
  @wrapErrors()
  async getAccountAlgoHours({
    periodStart,
    account,
  }: DelegatorContractArgs['getAccountAlgoHours(uint64,address)uint64']): Promise<bigint> {
    const { return: retVal } = await this.readClient.send.getAccountAlgoHours({ args: { account, periodStart } })
    return retVal!
  }

  @wrapErrors()
  async getAlgoHourPeriodTotals({
    periodStart,
  }: DelegatorContractArgs['getAlgoHourPeriodTotals(uint64)(uint64,bool)']): Promise<AlgohourPeriodTotals> {
