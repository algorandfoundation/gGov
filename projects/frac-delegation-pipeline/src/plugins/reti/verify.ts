 ...
 */
async function fetchLiveBoxes(indexer: Indexer, poolAppIds: Iterable<bigint>): Promise<Map<bigint, LiveBox>> {
    ...
    const entries = await Promise.all(
      ids.slice(offset, offset + BOX_FETCH_BATCH_SIZE).map(async (poolAppId) => {
        const box = await withRetry(() =>
          indexer.lookupApplicationBoxByIDandName(poolAppId, Buffer.from('stakers')).do(),
        )
...
export async function verifyAgainstChain(
  ...
  const liveBoxes = await fetchLiveBoxes(indexer, poolAppIds)
