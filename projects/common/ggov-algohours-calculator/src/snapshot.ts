 ...
 *
 * Usage:
 *   pnpm snapshot <round>                    # scan from genesis and write snapshots/<round>.json
 *   pnpm snapshot <round> --save-transfers   # also stream all transfers to snapshots/<round>.transfers.log
 *   pnpm snapshot <round> --check            # re-scan and diff against existing file (no write)
 *   pnpm snapshot <round> --inspect          # print supply stats from an existing snapshot (no scan)
 *
 * Env:
...
import { STALGO_APP_ADDRESS, STALGO_ASA_ID, TALGO_APP_ADDRESS, TALGO_ASA_ID } from './constants/tinyman'
...
async function main() {
  ...
  if (positionalArgs.length !== 1 || !/^\d+$/.test(positionalArgs[0])) {
    throw new Error('Usage: pnpm snapshot <round> [--save-transfers] [--check] [--inspect]')
  }
  ...
  if (!check && existsSync(storedPath)) {
    throw new Error(
      `Snapshot already exists: ${storedPath}\n` +
        `To verify it: pnpm snapshot ${targetRound} --check\n` +
  ...
  if (check) {
    ...
    const diffs = diffSnapshotBalances(balances, readSnapshot(targetRound))
    if (diffs.length === 0) {
