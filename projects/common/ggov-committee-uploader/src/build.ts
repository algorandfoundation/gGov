...
async function main() {
  ...
  const selected = selectors.length ? index.filter((c) => selectors.some((s) => matchesSelector(c, s))) : index

  if (!selected.length) {
    throw new Error(
      `No committees matched ${JSON.stringify(selectors)} (available: ${index.map((c) => `${c.fromRound}-${c.toRound}`).join(', ')})`,
    )
  }
  ...
  for (const committee of selected) {
    ...
    console.log(`${name}  members=${file.totalMembers}  votes=${file.totalVotes}  committeeId=${toBase64Id(file)}`)
  }
